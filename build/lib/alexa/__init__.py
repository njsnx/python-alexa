...
class Alexa():
    ...
    def get_intents(self):
        ...
        for intent_name, mappings in self._intent_mappings.items()
            map['intents'][intent_name] = {
                "intent": intent_name,
            }

            if self._intent_mappings[intent_name]:
                map['intents'][intent_name]['slots'] = []
                ...
                for var, slot in mappings.items()
                    ...
                    map['intents'][intent_name]['slots'].append(
                        {
                            "name": slot_name,
                            "type": slot_type
        ...
        if raw is not None:  # Confirm a event has been passed in
            if "session" in raw.keys():  # Check if the event has a session key
                ...
                if self.context:
                    ...
                    self.device_id = ['System']['device']['deviceId']
...
class Helper():
    ...
    def load_utterances(self, file=None):

        with open(file) as ut:
            utterances = json.loads(ut)

        return self.generate_utterances(utterances)
